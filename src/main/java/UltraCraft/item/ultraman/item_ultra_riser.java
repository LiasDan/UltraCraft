package UltraCraft.item.ultraman;
...
import UltraCraft.model.model_belt_plus;
import UltraCraft.model.tokuArmorModel;
...
public class item_ultra_riser extends ItemArmor implements IHasModel
{
	...
	public item_ultra_riser (String name,ArmorMaterial par2EnumArmorMaterial, int par3, String rider,item_form_change baseFormItem,Item head,Item torso,Item legs)
	{
	...
	public item_ultra_riser belt(boolean num)
	{
		belt=num;
		return this;
	}
	...
	public  boolean rendModle(Entity entity, int num)
	{
		if (num==2||num==5||num==7||num==1||num==3||num==6||num==8){
			...
			if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET)!= null){
				if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem()instanceof item_ultra_riser){
					item_ultra_riser belt =((item_ultra_riser)player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem());
					String rider = ((item_ultra_riser)player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem()).Rider;

					 if (num==4||num==9||num==10||num==11||num==12||num==13||num==14){

						return item_ultra_riser.get_Form_Item_tex(player.getItemStackFromSlot(EntityEquipmentSlot.FEET),1).get2ndLyer()!="blank"? true:false;
					} else{
						return false;
		...
		if (entity instanceof EntityLivingBase){
			...
			if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET)!= null){
				if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem()instanceof item_ultra_riser){
					...
					if (num==1){
								return Refercence.MODID+":textures/armor/"+rider+item_ultra_riser.get_Form_Item_tex(player.getItemStackFromSlot(EntityEquipmentSlot.FEET),1).getFormName()+ext;
						...
						return Refercence.MODID+":textures/armor/"+item_ultra_riser.get_Form_Item_tex(player.getItemStackFromSlot(EntityEquipmentSlot.FEET),1).get2ndLyer()+ext;
	...
	public ModelBiped getArmorModel(EntityLivingBase living, ItemStack stack, EntityEquipmentSlot slot, ModelBiped defaultModel)
	{
		if(!stack.isEmpty())
		{
			if(stack.getItem() instanceof ItemArmor)
			{
				...
				if(belt == true){
					armorModel.belt=new ItemStack(UltraItems.blanknoitem);
				}else if (get_Form_Item(stack,1).getBelt()==UltraItems.blanknoitem){
	...
	public static item_form_change get_Form_Item_tex(ItemStack itemstack,int SLOT)
	{
		if (!itemstack.hasTagCompound())
		{
			return  ((item_ultra_riser)itemstack.getItem()).Base_Form_Item;
		}else if (Item.getItemById(itemstack.getTagCompound().getInteger("slot"+SLOT))instanceof item_form_change){
			return (item_form_change) Item.getItemById(itemstack.getTagCompound().getInteger("slot"+SLOT));
		}else{
			return  ((item_ultra_riser)itemstack.getItem()).Base_Form_Item;
	...
	{
		return UltraItems.land_of_light_fragment == repair.getItem() ? true : super.getIsRepairable(toRepair, repair);
	}
}