...
public class UltraRenderLayer<T extends GeoAnimatable> extends GeoRenderLayer<T> {

    public UltraRenderLayer(GeoRenderer<T> renderer ) {
        super(renderer);
    }

    protected ResourceLocation getTextureResource(T animatable, int n, LivingEntity entity, UltraRiserItem belt, EquipmentSlot slot) {
        if (slot == EquipmentSlot.FEET)  return   ResourceLocation.fromNamespaceAndPath(UltraCraftCore.MODID, "textures/armor/belts/" +belt.getUnlimitedTextures(entity.getItemBySlot(EquipmentSlot.FEET), entity, belt.Rider, n + 1) + ".png");
        return   ResourceLocation.fromNamespaceAndPath(UltraCraftCore.MODID, "textures/armor/" +belt.getUnlimitedTextures(entity.getItemBySlot(EquipmentSlot.FEET), entity, belt.Rider, n + 1) + ".png");
    }
    @Nullable
    protected RenderType getRenderType(T animatable,int run,LivingEntity entity,UltraRiserItem belt,EquipmentSlot slot) {
